...
class EmailService:
    ...
    async def send_group_invite_email(
        ...
        """Send a study-group invitation email."""
        try:
            msg = MIMEMultipart("alternative")
            msg["Subject"] = f"You're invited to join {group_name} on EduSense"
            ...
            <html>
              <body style="font-family: Arial, sans-serif; color: #333;">
                <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
                  <h2 style="color: #2563EB;">Study Group Invitation</h2>
                  <p>Hi there,</p>
                  <p><strong>{inviter_name}</strong> invited you to join the study group <strong>{group_name}</strong> for <strong>{module}</strong>.</p>
            ...
            logger.info(f"Study group invite email sent to {email}")
            return True
        except Exception as e:
            logger.error(f"Failed to send study group invite email: {str(e)}")
            return False
