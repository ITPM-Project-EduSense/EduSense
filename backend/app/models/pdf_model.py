...
class PdfMaterial(Document):
    ...
    class Settings:
        name = "Pdf_materials"
        indexes = [
            # 🔥 Prevent duplicates (same user + same file)
            IndexModel([("user_id", 1), ("file_hash", 1)], unique=True)
        ]  # MongoDB collection name


class PdfConcept(Document):
    """
    ...
    class Settings:
        name = "Pdf_concepts"  # MongoDB collection name

class PdfVectorCollection(Document):
    """
